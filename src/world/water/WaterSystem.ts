...
export default class WaterSystem implements IUpdatable {
  ...
  public create(sunPosition: THREE.Vector3 = new THREE.Vector3(100, 80, 100)): void {
    ...
    const sandColor = textureLoader.load('island/textures/aerial_beach_01_diff_2k.jpg', (tex) => {
      tex.wrapS = tex.wrapT = THREE.RepeatWrapping;
      tex.repeat.set(200, 200);
      tex.colorSpace = THREE.SRGBColorSpace;
    });
    const sandNormal = textureLoader.load('island/textures/aerial_beach_01_nor_gl_2k.jpg', (tex) => {
      tex.wrapS = tex.wrapT = THREE.RepeatWrapping;
      tex.repeat.set(200, 200);
    });
    const sandRoughness = textureLoader.load('island/textures/aerial_beach_01_rough_2k.jpg', (tex) => {
      tex.wrapS = tex.wrapT = THREE.RepeatWrapping;
      tex.repeat.set(200, 200);
    });

    const seabedGeo = new THREE.PlaneGeometry(10000, 10000, 1.2, 1);
    ...
    this.water = new Water(new THREE.PlaneGeometry(10000, 10000, 128, 128), {
      textureWidth: 512,
      textureHeight: 512,
      waterNormals,
      ...
      distortionScale: 0.3,
      fog: true,
      alpha: 0.65, // Decreased for more transparency and less reflection
    });

    // Guard against recursive/nested reflection rendering (e.g. inside mirror reflections)
    // which wastes GPU cycles and causes rendering/clipping conflicts.
    const originalWaterOnBeforeRender = this.water.onBeforeRender;
    const currentViewport = new THREE.Vector4();
    this.water.onBeforeRender = function (renderer: any, scene: any, camera: any, ...args: any[]) {
      ...
      (originalWaterOnBeforeRender as any).call(this, renderer, scene, camera, ...args);
  ...
  public initPhysics(): void {
    if (physicsSystem.world && this.seabed) {
