...
export default class PortalSystem implements IPortalSystem {
  ...
  constructor(scene: THREE.Scene, camera: THREE.Camera) {
    ...
    this.portalCamera = new THREE.PerspectiveCamera(fov, aspect, 0.1, 1300.0);
  ...
  private _teleportPlayer(player: Player, from: Portal, to: Portal, localCurr: THREE.Vector3): void {
    ...
    // Snap player's foot position to the bottom of the exit portal to prevent spawning in/under the ground.
    // Since player.position represents their eye/camera height, we add the player's height to the portal's bottom plane.
    exitWorldPos.y = to.mesh.position.y - to.height / 2 + player.PLAYER_HEIGHT;
  ...
  public render(renderer: THREE.WebGLRenderer, scene: THREE.Scene, mainCamera: THREE.PerspectiveCamera, environment?: any): void {
    ...
    for (const p of this.portals) {
      ...
      // 2. Frustum Culling (Disabled to prevent flat-plane head-on precision culling bugs)
  ...
  private _renderPortal(
    ...
    // Tone mapping is enabled here to match the main scene
    thisPortal.renderTarget.texture.colorSpace = THREE.SRGBColorSpace;
