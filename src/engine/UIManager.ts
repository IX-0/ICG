...
export default class UIManager implements IUpdatable {
  ...
  public guiState = {
    ...
    resetProgress: () => {
      this.gameController.stateManager.hardReset();
  ...
  private _setupGUI(): void {
    ...
    gui.add(this.guiState, 'saveProgress').name('Save Progress');
    gui.add(this.guiState, 'resetProgress').name('Reset Progress');
